...
const Sock = BotConnect({
  logger: Pino({ level: 'silent' }), // Adjust log level as needed: 'silent', 'debug', 'fatal'
  ...
  getMessage: async (currentChat) => {
    const message = { conversation: 'Bot' };
    if (store) {
  ...
  Sock.ev.on("messages.upsert", async (messageEvent) => {
    try {
      ...
      for (let mek of messageEvent.messages) {
        ...
        console.log(`Received message in ${mek.key.remoteJid.includes('@g.us') ? 'group' : 'chat'}:`);
        console.log(`From: ${mek.key.participant || mek.key.remoteJid}`);
  ...
  Sock.ev.on("messages.upsert", async (messageEvent) => {
    try {
      ...
      for (let mek of messageEvent.messages) {
        ...
        // Log the message details
        console.log(`Received message in ${messageObject.isGroup ? 'group' : 'chat'}:`);
        console.log(`From: ${messageObject.sender}`);
        console.log(`Message: ${messageText}`);
...
Sock.ev.on('connection.update', async (update) => {
  ...
  if (qr) {
    try {
      ...
      qrcode.toString(qr, (error, qrString) => {
        if (error) {
          console.error(error);
        } else {
  ...
  if (connection === 'connecting') {
    ...
    try {
      ...
      console.log('❌ ERROR INSTALLING PLUGINS ', error);
    ...
    switch (statusCode) {
      ...
      case DisconnectReason.connectionClosed:
        ...
        syncdb().catch((error) => console.error('Database sync error:', error));
        break;
...
async function MakeSession(sessionId = SESSION_ID, sessionDir = __dirname + '/session', isOfficial = false) {
 ...
 function checkDockerfile(pattern, filePath) {
   return new Promise((resolve, reject) => {
 ...
 const officialName = 'Asta-Md';
 const dockerfilePattern = '/Astropeda/';
 const isOfficialProject = toBool(isOfficial || global.IS_ASTA || process.env.IS_ASTA, true) || (await checkDockerfile(dockerfilePattern, './assets/Dockerfile'));
 ...
 if (isOfficialProject) {
   ...
   if (sessionString && sessionString.startsWith('PId_')) {
     try {
       let pasteId = sessionString.replace('PId_', '');
       const PastebinAPI = require('pastebin-js');
       const pastebin = new PastebinAPI('ECRgNok5kmfqqPofmC4NwFM8J6rx3qSO');
       const pasteContent = await pastebin.getPaste(pasteId);
       console.log({ pasteId });
       sessionString = pasteContent && typeof pasteContent === 'string' ? Buffer.from(pasteContent, 'utf-8').toString('base64') : sessionString;
     } catch (error) {
       console.log("CAN'T GET SESSION FROM PASTE ID\nERROR : ", error);
     }
   }

   if (sessionString && /guru/gi.test(sessionString) && sessionString.length < 30) {
     try {
       let guruPasteUrl = global.gurupaste || 'https://pastebin.guruapi.tech/pastes?action=getpaste&id=';
       const { data: pasteData } = await axios.get(guruPasteUrl + sessionString);
       const pasteContent = pasteData && pasteData.content ? pasteData.content : false;
       let sessionObj = pasteContent ? decodeBase64(pasteContent) : {};
       const credentials = JSON.parse(sessionObj);
       fs.writeFileSync(sessionDir + '/creds.json', JSON.stringify(credentials, null, 2));
       log('\nCredentials saved successfully.');
     } catch (error) {
       log('EMPTY SESSION_ID FROM GURU SERVER\nPLEASE SCAN THE QR AGAIN FROM [ ' + global.scan + ' ]\n\n\nERROR: ', error);
     }
   } else if (sessionString && sessionString.length > 3 && sessionString.length < 20) {
     try {
       let { data: pasteData } = await axios.get('https://paste.c-net.org/' + sessionString);
       fs.writeFileSync(sessionDir + '/creds.json', decodeBase64(pasteData), 'utf8');
     } catch (error) {
     ...
     try {
       ...
       let sessionObj = decodeBase64(sessionString);
       const credentials = JSON.parse(sessionObj);

       if (credentials['creds.json']) {
         for (const file in credentials) {
           try {
             fs.writeFileSync(sessionDir + '/' + file, typeof credentials[file] === 'string' ? credentials[file] : JSON.stringify(credentials[file], null, 2));
           } catch (error) {}
         }
       } else {
         fs.writeFileSync(sessionDir + '/creds.json', JSON.stringify(credentials, null, 2));
   ...
   AmdOfficial = false;
   log('\n\nA Cheap Copy of ' + officialName + ' was found.\nDeploy From : https://github.com' + dockerfilePattern + '' + officialName + '\n');
   process.exit(0);
 }
}
async function main() {
